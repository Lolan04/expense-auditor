...
export default function Dashboard() {
  ...
  const openDetail = async (id) => {
    ...
    try {
      const r = await fetch(`${API}/api/receipts/${id}`);
  ...
  const submitOverride = async () => {
    if (!override.new_verdict || !override.comment || !override.auditor_name) {
      setOverrideMsg("Please fill all fields.");
      return;
    }
    try {
      ...
      if (r.ok) {
        ...
        setOverrideMsg("Failed: " + (e.detail || "Unknown error"));
      }
    } catch {
    ...
    <div style={{ minHeight:"100vh", background:"#f0f2f5", fontFamily:"Arial, sans-serif" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1a1a2e,#16213e)", padding:"20px 32px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <button
          ...
          style={{ background:"rgba(255,255,255,0.15)", color:"#fff", border:"1px solid rgba(255,255,255,0.3)", padding:"8px 18px", borderRadius:8, cursor:"pointer", fontSize:14 }}
        >
          Refresh
        </button>
      </div>
      ...
      <div style={{ padding:"24px 32px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
          {[
            ["Total Claims", stats.total, "#6c757d"],
            ["Approved",     stats.approved, "#28a745"],
            ["Flagged",      stats.flagged,  "#ffc107"],
        ...
        <div style={{ marginBottom:16 }}>
          {["All","Rejected","Flagged","Approved"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              style={{ padding:"7px 18px", borderRadius:20, border:"none", cursor:"pointer", fontWeight:600, fontSize:13, marginRight:8, background:filter===f?"#0f3460":"#e9ecef", color:filter===f?"#fff":"#495057" }}
            >
              {f}
            </button>
          ))}
        ...
        <div style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr>
                {["#","Employee","Merchant","Amount","Date","Verdict","Action"].map((h) => (
                  <th key={h} style={{ background:"#f8f9fa", padding:"12px 16px", textAlign:"left", fontSize:13, fontWeight:600, color:"#495057", borderBottom:"1px solid #dee2e6" }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ textAlign:"center", padding:40, color:"#999" }}>Loading...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign:"center", padding:40, color:"#999" }}>No claims found</td>
                </tr>
              ) : (
                filtered.map((r) => (
                  <tr key={r.id}>
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                      <strong style={{ color:"#6c757d" }}>#{r.id}</strong>
                    </td>
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                    ...
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                      <strong>{r.ocr_amount} {r.ocr_currency}</strong>
                    </td>
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                      {r.expense_date}
                    </td>
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                      <span style={{ display:"inline-block", padding:"3px 10px", borderRadius:12, fontSize:12, fontWeight:700, background:VC[r.ai_verdict]?.bg, color:VC[r.ai_verdict]?.color }}>
                        {r.ai_verdict}
                      </span>
                      {r.override_by && (
                        <div style={{ fontSize:11, color:"#888", marginTop:3 }}>Overridden by {r.override_by}</div>
                      )}
                    </td>
                    <td style={{ padding:"12px 16px", fontSize:14, borderBottom:"1px solid #f0f0f0" }}>
                      <button
                        onClick={() => openDetail(r.id)}
                        style={{ padding:"6px 14px", background:"#0f3460", color:"#fff", border:"none", borderRadius:6, cursor:"pointer", fontSize:13 }}
                      >
                        View Details
                      </button>
                    </td>
      ...
      {selected && (
        <div
          style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.5)", zIndex:1000, display:"flex", alignItems:"flex-start", justifyContent:"center", overflowY:"auto", padding:"40px 20px" }}
          ...
          <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:900, boxShadow:"0 30px 80px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ background:"linear-gradient(135deg,#667eea,#764ba2)", padding:"20px 28px", borderRadius:"16px 16px 0 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:"#fff" }}>
                ...
                {detail && (
                  <div style={{ fontSize:13, opacity:0.85 }}>{detail.employee_name} · {detail.expense_date}</div>
                )}
              </div>
              <button
                onClick={() => setSelected(null)}
                style={{ background:"rgba(255,255,255,0.2)", border:"none", color:"#fff", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:16 }}
              >
                Close
              </button>
            </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24 }}>
                  ...
                  <div>
                    ...
                    {detail.receipt_image_base64 ? (
                      <img
                        src={`data:image/png;base64,${detail.receipt_image_base64}`}
                        alt="receipt"
                        style={{ width:"100%", borderRadius:10, border:"1px solid #ddd" }}
                      />
                    ) : (
                      <div style={{ background:"#f0f0f0", height:200, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", color:"#999" }}>
                        No image
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <div style={{ marginBottom:16 }}>
                      ...
                      <div style={{ background:"#f8f9fa", padding:"12px 16px", borderRadius:8, fontSize:14, color:"#555" }}>
                        {detail.description}
                      </div>
                    </div>
                    ...
                    <div style={{ background:"#f0f4ff", borderRadius:10, padding:16, border:"1px solid #c7d2fe" }}>
                      ...
                      <select
                        value={override.new_verdict}
                        onChange={(e) => setOverride({...override, new_verdict:e.target.value})}
                      ...
                      <input
                        value={override.auditor_name}
                        onChange={(e) => setOverride({...override, auditor_name:e.target.value})}
                        placeholder="Your name (auditor)"
                        style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:"1.5px solid #c7d2fe", fontSize:14, boxSizing:"border-box", marginBottom:10 }}
                      />
                      <textarea
                        value={override.comment}
                        onChange={(e) => setOverride({...override, comment:e.target.value})}
                      ...
                      {overrideMsg && (
                        <div style={{ marginTop:8, fontSize:13, color:overrideMsg.startsWith("Override")?"#155724":"#721c24" }}>
                          {overrideMsg}
                        </div>
                      )}
                    </div>
